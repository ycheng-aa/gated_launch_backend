    ...
    def filter_fields(cls, data, values, values_white_list=True):
        if isinstance(values, list):
            ...
            if values is not None:
                for filter_key in values:
                    if "." in filter_key:
                        new_key = filter_key.split(".")
                        if new_key[0] not in new_filter:
                            new_filter[new_key[0]] = []
                        new_filter[new_key[0]].append(new_key[1])
            for item in data:
                _item = {}
                for key, value in item.items():
                    if (key in values and values_white_list) or (key not in values and not values_white_list):
                        _item[key] = value
                if new_filter:
                    for field_key, field_value in new_filter.items():
                        _item[field_key] = []
                        for data_list in item[field_key]:
                            new_value = {}
                            for value_list in field_value:
                                new_value[value_list] = data_list[value_list]
                            _item[field_key].append(new_value)
                res.append(_item)
            return res
        return data
