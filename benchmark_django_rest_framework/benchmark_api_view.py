...
class BenchmarkAPIView(PARENT_VIEW):
    ...
    def as_view(cls, actions=None, **initkwargs):
        ...
        if SETTINGS.PARENT_VIEW in ('ViewSet', 'ModelViewSet'):
            ...
            if actions is None:
                ...
                if 'all' not in cls.access.values():
    ...
    def process_response(self, res):
        if isinstance(res, dict):    # dict 转 json 返回
            ...
            # process json response style step 1: json data
            json_response_style = getattr(SETTINGS, 'JSON_RESPONSE_STYLE', None)
            if json_response_style == 'gray':
                code = res[SETTINGS.CODE]
                msg = res[SETTINGS.MSG]
                res = res[SETTINGS.DATA] if isinstance(res[SETTINGS.DATA], dict) else {}
                if code != SETTINGS.SUCCESS_CODE:
                    res['detail'] = msg
            elif json_response_style != 'benchmark':
                raise Exception('JSON_RESPONSE_STYLE in the benchmark_settings is not defined or not correct. The value of it should be "benchmark", or "gray"')
            ...
            # process json response style step 2: http status code
            if json_response_style == 'gray' and code != SETTINGS.SUCCESS_CODE:
                if code - SETTINGS.CODE_OFFSET == 21:
                    res.status_code = status.HTTP_401_UNAUTHORIZED
                elif code - SETTINGS.CODE_OFFSET in (22, 23):
